...
struct JSValueWrapper<T> {
    JSValue Wrap(JSContext* ctx, T& value) {
        JSClassID id = QJSClassIDManager<T>::GetOrGen().m_id;
    ...
    T& Unwrap(JSContext* ctx, const JSValue& value) {
        ...
        return *static_cast<T*>(
            JS_GetOpaque(value, QJSClassIDManager<T>::GetOrGen().m_id));
    }
};
...
struct JSValueWrapper<const T> {
    JSValue Wrap(JSContext* ctx, const T& value) {
        // FIXME: maybe copy in ?
        JSClassID id = QJSClassIDManager<T>::GetOrGen().m_const_id;
        JSValue obj = JS_NewObjectClass(ctx, id);
        if (JS_IsException(obj)) {
            LogJSException(ctx);
        }
    ...
    T& Unwrap(JSContext* ctx, const JSValue& value) {
        NICKEL_ASSERT(JS_IsObject(value), "js value is not object");
        return *static_cast<T*>(
            JS_GetOpaque(value, QJSClassIDManager<T>::GetOrGen().m_const_id));
    }
};
...
struct JSValueWrapper<const T&> {
    JSValue Wrap(JSContext* ctx, const T& value) {
        JSValue obj = JS_NewObjectClass(
            ctx, QJSClassIDManager<T>::GetOrGen().m_const_ref_id);
        if (JS_IsException(obj)) {
            LogJSException(ctx);
        }
        QJS_CALL(ctx, JS_SetOpaque(obj, &value));
...
struct JSValueWrapper<T&> {
    JSValue Wrap(JSContext* ctx, T& value) {
        JSValue obj =
            JS_NewObjectClass(ctx, QJSClassIDManager<T>::GetOrGen().m_ref_id);
        if (JS_IsException(obj)) {
            LogJSException(ctx);
        }
        QJS_CALL(ctx, JS_SetOpaque(obj, &value));
...
struct JSValueWrapper<const T> {
    JSValue Wrap(JSContext* ctx, const T& value) {
        JSValue obj =
            JS_NewObjectClass(ctx, QJSClassIDManager<T>::GetOrGen().m_const_id);
        if (JS_IsException(obj)) {
            LogJSException(ctx);
        }
        QJS_CALL(ctx, JS_SetOpaque(obj, (void*)&value));
...
struct JSValueWrapper<const T*> {
    JSValue Wrap(JSContext* ctx, const T* value) {
        JSValue obj = JS_NewObjectClass(
            ctx, QJSClassIDManager<T>::GetOrGen().m_const_pointer_id);
        if (JS_IsException(obj)) {
            LogJSException(ctx);
        }
        QJS_CALL(ctx, JS_SetOpaque(obj, value));

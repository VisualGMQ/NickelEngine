    ...
    auto vertShader =
        device
            .CreateShaderModule(vk::ShaderStageFlagBits::eVertex,
                                "test/testbed/vulkan/02triangle/vert.spv")
            .value;
    auto fragShader =
        device
            .CreateShaderModule(vk::ShaderStageFlagBits::eFragment,
                                "test/testbed/vulkan/02triangle/frag.spv")
            .value;
...
void StartupSystem(gecs::commands cmds,
    ...
    for (auto view : swapchain.ImageViews()) {
        ...
        ctx.graphicsFinishSemaphore.emplace_back(device.CreateSemaphore().value);
        ctx.waitImageAvaliableSemaphore.emplace_back(device.CreateSemaphore().value);
        ctx.cmds.emplace_back(ctx.cmdPool->Allocate(vk::CommandBufferLevel::ePrimary, 1)[0]);
...
void UpdateSystem(gecs::resource<gecs::mut<nickel::vulkan::Device>> device,
    ...
    auto index = device->GetSwapchain().AcquireNextImage(
        {}, ctx->waitImageAvaliableSemaphore[ctx->frameIndex], nullptr/*ctx->fence*/);
    ...
    vk::Semaphore waitImageSem = *ctx->waitImageAvaliableSemaphore[ctx->frameIndex];
    vk::Semaphore graphicsFinishSem = *ctx->graphicsFinishSemaphore[ctx->frameIndex];
...
void BootstrapSystem(gecs::world& world,
    ...
    nickel::Window& window =
        reg.commands().emplace_resource<nickel::Window>("02 triangle", 1024, 720);

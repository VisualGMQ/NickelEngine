...
void initInstanceUniformBuffer(Context& ctx, Device& device) {
    ...
    bufferDesc.usage = BufferUsage::Uniform;
    bufferDesc.mappedAtCreation = true;
    bufferDesc.size = 3 * 4 * InstanceCount;
    ctx.instanceUniformBuffer = device.CreateBuffer(bufferDesc);

    constexpr float step = 3;
    std::array<nickel::cgmath::Vec3, InstanceCount> offsets;
    uint32_t row = std::sqrt(InstanceCount);
    for (int x = 0; x < row; x++) {
        for (int y = 0; y < row; y++) {
            offsets[x + y * row] = nickel::cgmath::Vec3{x * step, y * step, 0};
        }
    }

    void* data = ctx.instanceUniformBuffer.GetMappedRange();
...
void StartupSystem(gecs::commands cmds,
    ...
    bufferState.stepMode = RenderPipeline::BufferState::StepMode::Vertex;

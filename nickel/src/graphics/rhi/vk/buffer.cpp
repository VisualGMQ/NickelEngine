...
vk::MemoryPropertyFlags BufferImpl::getMemoryProperty(
    ...
    uint32_t prop = 0;
    uint32_t usageBits = static_cast<uint32_t>(desc.usage);
    if (usageBits & static_cast<uint32_t>(BufferUsage::MapRead) ||
        usageBits & static_cast<uint32_t>(BufferUsage::MapWrite) ||
...
void BufferImpl::MapAsync(Buffer::Mode mode, uint64_t offset, uint64_t size) {
